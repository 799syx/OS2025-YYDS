...
found:
  ...
  p->alarm_ticks = 0;
  p->alarm_goingoff = 0;
  p->uid = 0;  // 默认用户ID为0 (root)
  p->gid = 0;  // 默认组ID为0 (root)
  p->umask = 022;  // 默认umask为022
  p->pgid = p->pid;  // 默认进程组ID为自身PID
  p->sid = p->pid;   // 默认会话ID为自身PID
  p->pending_signals = 0;  // 无待处理信号
  p->paused = 0;
  
  // MLFQ 初始化：新进程从最高优先级队列开始
  p->mlfq_level = 0;
  p->mlfq_ticks = 0;
...
int fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->uid = p->uid;  // 从父进程复制uid到子进程
  np->gid = p->gid;  // 从父进程复制gid到子进程
  np->umask = p->umask;  // 从父进程复制umask到子进程
  np->pgid = p->pgid;  // 继承父进程的进程组ID
  np->sid = p->sid;    // 继承父进程的会话ID
  np->pending_signals = 0;  // 子进程不继承待处理信号
  np->paused = 0;
  for (int i = 0; i < 32; i++)
    np->signal_handlers[i] = p->signal_handlers[i];  // 继承信号处理函数
  release(&np->lock);
...
// 设计特点：
// 1. 4个优先级队列 (0=最高, 3=最低)
// 2. 每个队列有不同的时间片: Q0=2, Q1=4, Q2=8, Q3=16 ticks
// 3. 每个级别有时间配额，用完后降级
// 4. 定期进行优先级提升(Priority Boost)防止饥饿
// 5. I/O密集型进程（主动让出CPU）保持高优先级

#define MLFQ_LEVELS 4
#define MLFQ_BOOST_INTERVAL 100  // 每100个调度周期进行一次优先级提升

// 每个级别的时间配额
static int mlfq_allotment_init[MLFQ_LEVELS] = {5, 10, 20, 40};  // 第4级配额较大
...
void mlfq_priority_boost(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    if (p->state != UNUSED) {
      p->mlfq_level = 0;
      p->mlfq_ticks = 0;
      p->mlfq_allotment = mlfq_allotment_init[0];
    }
    release(&p->lock);
  }
}
...
run_process:
    ...
    if (selected->state == RUNNABLE)
    {
      ...
      selected->mlfq_ticks = 0;  // 重置当前时间片计数
      c->proc = selected;
...
// If pid == -1, wait for any child (same as wait()).
